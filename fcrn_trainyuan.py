                        ...
                        for train_c in range(TRAIN_CYCLE):
                            ...
                            if (train_c+1)%500==0:
                                ...
                                for test_c in range(TEST_CYCLE):
                                    l_test, a_test, test_m, test_v, test_pre,test_lab = sess.run(
                                        [loss_test, acc_test, test_mean, test_var, logits_test,label_test_batch])
                                    l_teall += l_test
                                    a_teall += a_test
                                    mean_tall += test_m
                                    var_tall += test_v
                                    for i, (p_depth, gro_depth) in enumerate(zip(test_pre, test_lab)):
                                        p_depth = tf.squeeze(p_depth)
                                        gro_depth = tf.squeeze(gro_depth)
                                        p_dep = p_depth.eval()
                                        pil = gro_depth.eval()
                                        print(type(p_dep))
                                        print(type(pil))
                                        p_dep = Image.fromarray(np.uint8(p_dep))
                                        pil = Image.fromarray(np.uint8(pil))
                                        p_dep.save('./fcrn_data/%d_pre.png' % i)
                                        pil.save('./fcrn_data/%d_gro.png' % i)
