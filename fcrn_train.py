...
def inference(inputs,istrain,reuse):
    with tf.variable_scope('model',reuse=reuse):
        ...
        net=Conv2d(relu2a,512,(1,1),(2,2),b_init=None,name='res3a_branch1')
        bn3a=BatchNormLayer(net,0.999,0.00001,None,istrain,name='bn3a_branch1')
        net=Conv2d(relu2a,128,(1,1),(2,2),b_init=None,name='res3a_branch2a')
        ...
        net = Conv2d(relu3a, 128, (1, 1), (1, 1), b_init=None, name='res3d_branch2a')
        ...
        res3d=ElementwiseLayer([relu3a,net],combine_fn=tf.add, name='res3d')
        ...
        # net = Conv2d(relu4a, 256, (1, 1), (1, 1), b_init=None, name='res4e_branch2a')
        ...
        # res4e = ElementwiseLayer([relu4a, net], combine_fn=tf.add, name='res4e')
        ...
        net=Conv2d(relu4a,2048,(1,1),(2,2),b_init=None,name='res5a_branch1')
        bn5a=BatchNormLayer(net,0.999,0.00001,None,istrain,name='bn5a_branch1')
        net = Conv2d(relu4a, 512, (1, 1), (2, 2), b_init=None, name='res5a_branch2a')
        ...
        # net = Conv2d(relu5a, 512, (1, 1), (1, 1), b_init=None, name='res5c_branch2a')
        ...
        # res5c= ElementwiseLayer([relu5a, net], combine_fn=tf.add, name='res5c')
        # relu5c = LambdaLayer(res5c, tf.nn.relu, name='res5c_relu')

        net = Conv2d(relu5a, 1024, (1, 1), (1, 1),act=None ,name='layer1')
        net = DropoutLayer(net, 0.5, True, is_train=istrain, name='drop2')
...
if __name__ == '__main__':
    ...
    img_train,label_train  = read_and_decode_test(TRAIN_TFRECORD)
    img_test,label_test  = read_and_decode_test(TEST_TFRECORD)
    ...
    tf.summary.scalar('z_loss_test',loss_test)

    tf.summary.image("image_train", img_train_batch)
    tf.summary.image("pre_train", logits_train)
    tf.summary.image("label_train,",label_train_batch)

    tf.summary.image("z_image_test", img_test_batch)
    tf.summary.image("z_pre_test", logits_test)
    tf.summary.image("z_label_test", label_test_batch)
        ...
        try:
            while not coord.should_stop():
                ...
                while run_cycle < MAX_Cycle:
                    ...
                    if test_mode == False:
                        for train_c in range(TRAIN_CYCLE):
                            _,l_train,a_train,train_m,train_v,train_pre=sess.run([train,loss_train,acc_train,train_mean,train_var,logits_train])
                            l_tall+=l_train
                            a_tall+=a_train
                            mean_all += train_m
                            var_all += train_v
                            if (train_c+1)%100==0:
                                print("epoch:%d,step:%d"%(run_cycle,train_c))
                                print('train_loss:%f'%(l_tall/100.))
                                print('train_acc:%f'%(a_tall/100.))
                                print('train_mean_all:%f' % (mean_all / 100.))
                                print('train_var_all:%f' % (var_all / 100.))
                            ...
                            if (train_c+1)%500==0:
                                ...
                                for test_c in range(TEST_CYCLE):
                                    l_test, a_test, test_m, test_v, test_pre = sess.run(
                                        [loss_test, acc_test, test_mean, test_var, logits_test])
                                    l_teall += l_test
                                    a_teall += a_test
                                    mean_tall += test_m
                                    var_tall += test_v
                    ...
                    if test_mode == True:
                        for test_c in range(TEST_CYCLE):
                            l_test,a_test,test_m,test_v,test_pre=sess.run([loss_test,acc_test,test_mean,test_var,logits_test])
                            l_teall+=l_test
                            a_teall+=a_test
                            mean_tall += test_m
                            var_tall += test_v
